...
function makeFallbackTexture(gl) {
  ...
  for (let i = 0; i < size * size; i++) {
    data[i * 4] = 24; data[i * 4 + 1] = 24; data[i * 4 + 2] = 28; data[i * 4 + 3] = 255;
  }
...
class MorphEngine {
  ...
  goTo(dir) {
    ...
    this.tween = gsap.fromTo(this.program.uniforms.uProgress, { value: 0 }, { value: 1, duration, ease: opts.ease, onComplete: () => this.commit(target) });
  }
  ...
  goToIndex(idx) {
    ...
    this.goTo(idx > this.current ? 1 : -1);
  }
...
/**
 * MorphSlider — GPU-driven WebGL morph transition slider.
 * Extended with:
 *   - onSlideChange prop: fires whenever the visible slide changes
 *   - forwardRef + useImperativeHandle exposing { next, prev, goTo }
 *     so parent components can drive navigation imperatively
 */
const MorphSlider = forwardRef(function MorphSlider({
  ...
  useImperativeHandle(ref, () => ({
    ...
    goTo: (i) => engineRef.current?.goToIndex(i),
  }), []);
  ...
  useEffect(() => {
    ...
    const onDown = e => {
      ...
      if (active && el.setPointerCapture) { try { el.setPointerCapture(e.pointerId); } catch {} }
    };
    const onMove = e => { if (!active) return; engineRef.current?.drag((e.clientX - startX) / width); };
        ...
        <div className="morph-slider-indicators" role="tablist" aria-label="Slides">
          {items.map((item, i) => (
            <button
              ...
              onClick={() => { const engine = engineRef.current; if (!engine || i === index) return; engine.goTo(i > index ? 1 : -1); }}
            />
          ))}
        </div>
      )}
